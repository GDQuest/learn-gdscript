...
func _generate_validation_script() -> void:
	assert(
		"return" in validation_script,
		"Your validation script does not return anything (%s)" % [get_path()]
	)
...
func validate(script_text: String) -> bool:
	return validation_script_instance.validate(script_text)
