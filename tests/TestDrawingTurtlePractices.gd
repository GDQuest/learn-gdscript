...
const DrawingTurtleScript := preload("res://game_demos/DrawingTurtle.gd")
...
func _test_lesson7_practice2_position_solution() -> void:
	# Official solution using position.x assignments
...
func _test_lesson7_practice2_jump_solution() -> void:
	# Alternative student solution using jump()
	var turtle := _create_turtle()
...
func _test_lesson7_practice2_mixed_solution() -> void:
	# Mixed solution using both position and jump
	var turtle := _create_turtle()
...
func _test_position_change_closes_polygon() -> void:
	...
	# Change position mid-drawing - should close the incomplete polygon
	turtle.position = Vector2(200, 200)
	turtle.move_forward(30)

	var polygons = turtle.get_polygons()

	# The first incomplete shape should be closed when position changes
...
func _test_jump_after_position_change() -> void:
	...
	# After rectangle, position should be back at (100, 100)
	# Now jump should work relative to that position
	turtle.jump(100, 0)
	_draw_rectangle(turtle, 50, 50)
...
func _test_multiple_position_changes_before_drawing() -> void:
	...
	# Multiple position changes before drawing shouldn't create spurious polygons
	...
	# Should only have 1 real polygon (the rectangle)
...
func _test_polygon_points_are_normalized() -> void:
	...
	# The points getter should normalize points relative to the first point
...
func _test_position_and_jump_mix_does_not_leave_unclosed_shapes() -> void:
	# Regression test: Previously, mixing position changes and jumps would
	# leave shapes unclosed
	var turtle := _create_turtle()
	...
	# Verify each polygon is a closed square (5 points: 4 corners + return to start)
	for i in range(polygons.size()):
		var points := Array(polygons[i].points)
		_assert_true(
			points.size() == 5,
...
func _draw_rectangle(turtle: Node2D, length: float, height: float) -> void:
	...
	turtle.turn_right(90)
	turtle.move_forward(height)
	turtle.turn_right(90)
	turtle.move_forward(length)
	turtle.turn_right(90)
	turtle.move_forward(height)
	turtle.turn_right(90)
	turtle._close_polygon()
...
func _assert_polygon_is_100x100_square(polygon, message: String) -> void:
	...
	var actual_points := Array(polygon.points)
	actual_points.sort()
