...
func _ready() -> void:
	_add_block_button.pressed.connect(Callable(self, "_on_add_block_pressed"))
	_add_quiz_button.pressed.connect(Callable(self, "_on_add_quiz_pressed"))
	_cancel_button.pressed.connect(Callable(self, "_on_cancelled"))
