...
func _ready() -> void:
	...
	_options_block_type.item_selected.connect(Callable(self, "_on_options_block_type_item_selected"))

	_select_file_button.pressed.connect(Callable(self, "_on_select_file_requested"))
	_clear_file_button.pressed.connect(Callable(self, "_on_clear_file_requested"))
	_visual_element_value.text_changed.connect(Callable(self, "_update_visual_element_file"))
	_visuals_on_left_checkbox.toggled.connect(Callable(self, "_on_visuals_on_left_toggled"))

	_text_content_value.text_changed.connect(Callable(self, "_on_text_content_changed"))
	_text_content_value.gui_input.connect(Callable(self, "_on_text_content_value_gui_input"))
	_text_content_expand_button.pressed.connect(Callable(self, "_open_expanded_text_box"))
	_text_edit_dialog.confirmed.connect(Callable(self, "_on_text_content_confirmed"))
...
func search(search_text: String, from_line := 0, from_column := 0) -> Vector2i:
	var result: Vector2i = _text_content_value.search(
		search_text,
	...
	if result.x != -1:
		...
		_text_content_value.select(
			result.x,
			result.y,
			result.x,
			result.y + search_text.length()
		)
