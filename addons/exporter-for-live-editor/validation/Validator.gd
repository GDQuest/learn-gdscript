...
# The signal is an array of errors. If the array is empty, the
# validation is considered successful.
# Use the `_validation_success()` and `_validation_error([])` methods
...
# @abstract
# Use this method for small verifications along the way inside a validation
# This is equivalent to an `assert`, but it dispatches 
# `validation_completed` with errors if wrong.
# 
# Example Usage:
# 
# ```
