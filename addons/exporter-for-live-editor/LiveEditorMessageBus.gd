...
var script_replacements := RegExp.collection({
	"\\b(?<command>prints)\\((?<args>.*?)\\)": "LiveEditorMessageBus.print_log([{args}], {line}, {char})",
	"\\b(?<command>print)\\((?<args>.*?)\\)": "LiveEditorMessageBus.print_log([{args}], {line}, {char})",
	"\\b(?<command>push_error)\\((?<args>.*?)\\)": "LiveEditorMessageBus.print_error({args}, {line}, {char})",
	"\\b(?<command>push_warning)\\((?<args>.*?)\\)": "LiveEditorMessageBus.print_warning({args}, {line}, {char})",
	"\\b(?<command>assert)\\((?<args>.*?)\\)": "LiveEditorMessageBus.print_assert({args}, {line}, {char})",
})
...
func print_assert(assertion: bool, provided_message := "", line_nb: int = 0, character: int = 0) -> void:
	var message = "" if assertion else \
		"Assertion failed" if provided_message == "" else provided_message 
	if not assertion:
