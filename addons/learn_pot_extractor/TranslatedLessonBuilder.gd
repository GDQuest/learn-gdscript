...
static func build_translated_lesson(lesson_bbcode_path: String, lang: String) -> void:
	...
	var tr_blocks := SHARED.build_tr_blocks("res://i18n/course.%s.po" % [lang])
	
	var lesson_builder: Array[String] = []
	_build_lesson_tag(lesson_builder, lesson, tr_blocks)
	...
	for child: BBCodeParser.ParseNode in lesson.children:
		match child.tag:
			BBCodeParserData.Tag.TITLE:
				_build_title_tag(lesson_builder, lesson, child, tr_blocks)
			BBCodeParserData.Tag.VISUAL:
				_build_visual_tag(lesson_builder, child)
			BBCodeParserData.Tag.NOTE:
				_build_note_tag(lesson_builder, child, tr_blocks)
			BBCodeParserData.Tag.QUIZ_CHOICE:
				_build_quiz_choice_tag(lesson_builder, child, tr_blocks)
			BBCodeParserData.Tag.QUIZ_INPUT:
				pass
			BBCodeParserData.Tag.PARAGRAPH:
				_build_text(lesson_builder, child, tr_blocks)
			BBCodeParserData.Tag.PRACTICE:
				_build_practice_tag(lesson_builder, child, tr_blocks)
...
static func _build_lesson_tag(str_builder: Array[String], lesson: BBCodeParser.ParseNode, tr_blocks: Array[Dictionary]) -> void:
	str_builder.append_array(['[lesson title="%s"]' % [_tr(BBCodeUtils.get_lesson_title(lesson).replace('"', r'\"'), tr_blocks)], ""])


static func _build_title_tag(str_builder: Array[String], lesson: BBCodeParser.ParseNode, title: BBCodeParser.ParseNode, tr_blocks: Array[Dictionary]) -> void:
	str_builder.append_array(["[title]%s[/title]" % [_tr(BBCodeUtils.get_lesson_title_for_index(lesson, lesson.children.find(title)), tr_blocks)], ""])
...
static func _build_note_tag(str_builder: Array[String], note: BBCodeParser.ParseNode, tr_blocks: Array[Dictionary]) -> void:
	str_builder.append('[note title="%s"]' % [_tr(BBCodeUtils.get_note_title(note).replace('"', r'\"'), tr_blocks)])
	
	var note_lines := _get_lines(BBCodeUtils.get_note_contents(note))
	for i in note_lines.size():
		var line: String = note_lines[i]
		str_builder.append(_tr(line, tr_blocks))
...
static func _build_quiz_choice_tag(str_builder: Array[String], quiz: BBCodeParser.ParseNode, tr_blocks: Array[Dictionary]) -> void:
	var quiz_data := BBCodeUtils.get_quiz_data(quiz)
	str_builder.append('[quiz_choice question="%s" multiple="%s" shuffle="%s"]' % [_tr(quiz_data.question.replace('"', r'\"'), tr_blocks), quiz_data.multiple, quiz_data.shuffle])
	...
	for answer: String in quiz_data.answers:
		str_builder.append("[option%s]%s[/option]" % [" correct" if quiz_data.valid_answers.has(answer) else "", _tr(answer, tr_blocks)])
	
	var explanation_lines := _get_lines(quiz_data.explanation)
	if explanation_lines.size() == 0:
		str_builder.append("[explanation]%s[/explanation]" % [_tr(explanation_lines[0], tr_blocks)])
	else:
...
static func _build_text(str_builder: Array[String], text_node: BBCodeParser.ParseNode, tr_blocks: Array[Dictionary]) -> void:
	var text := BBCodeUtils.get_paragraph_text(text_node)
	var lines := _get_lines(text)
	for i in lines.size():
		var line := lines[i]
		str_builder.append(_tr(SHARED.fix_glossary_entries(line, []), tr_blocks))
		if i < lines.size()-1:
			str_builder.append("")
	str_builder.append("")


static func _build_practice_tag(str_builder: Array[String], practice: BBCodeParser.ParseNode, tr_blocks: Array[Dictionary]) -> void:
	str_builder.append('[practice id="%s" title="%s"]' % [BBCodeUtils.get_practice_id(practice), _tr(BBCodeUtils.get_practice_title(practice).replace('"', r'\"'), tr_blocks)])
	
	var description_lines := _get_lines(BBCodeUtils.get_practice_description(practice))
	if description_lines.size() == 1:
		str_builder.append("[description]%s[/description]" % [_tr(description_lines[0], tr_blocks)])
	else:
	...
	if goal_lines.size() == 1:
		str_builder.append("[goal]%s[/goal]" % [_tr(goal_lines[0], tr_blocks)])
	else:
	...
	for hint in hints:
		str_builder.append("[hint]%s[/hint]" % [_tr(hint, tr_blocks)])
	
	str_builder.append("[/practice]")


static func _tr(original: String, tr_blocks: Array[Dictionary]) -> String:
