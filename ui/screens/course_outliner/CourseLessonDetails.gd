...
# Combined properties and logic. When you set 'lesson', visuals update automatically.
var lesson: Lesson:
	set(value):
		lesson = value
		_update_visuals()
...
# FIX: Removed _reading_stats_block (Unused variable warning)
...
# FIX: Removed _practice_stats_block (Unused variable warning)
...
# Note: The separate set_lesson, set_lesson_progress, etc. functions 
# were removed because their logic is now handled by the properties above.

func _update_visuals() -> void:
	if not is_inside_tree() or not lesson:
		return
	...
	if lesson_progress:
		...
		if lesson_progress.completed_reading or completed_blocks >= total_blocks:
			...
			# Use float casting to ensure percentage calculation works
			var progress_percent := int((float(completed_blocks) / float(total_blocks)) * 100.0)
			_reading_stats_value.text = "%d%%" % progress_percent
