...
## Returns the lesson path based on its position in this course (position
## starting from 1, not 0).
func get_lesson_path_from_number(lesson_number: int) -> String:
	var lesson_index := lesson_number - 1
	if lesson_index < 0 or lesson_index >= LESSONS.size():
		return ""
	return get_lesson_path(lesson_index)
...
func get_lesson_path_from_slug(slug: String) -> String:
	return _inverted_lessons.get(slug, "")
