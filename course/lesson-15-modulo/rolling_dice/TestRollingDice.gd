...
func test_randi_function_used_with_sides() -> String:
	...
	if not result:
		return tr("It looks like modulo isn't used correctly in the script. Make sure to use modulo (%) in the correct order.")
	return ""
...
func test_function_simulates_rolling_dice() -> String:
	...
	if results.min() > 7:
		return tr("It looks like the minimum number the die can roll is too high. Did you add too much to the result?")
	
	if not results.hash() == EXPECTED_DICE_SEQUENCE.hash():
		return tr("Something is wrong with the roll function.")
	return ""
